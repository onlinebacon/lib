...
export const orthographic = new Projection({
	name: 'Orthographic',
	ratio: 1,
	azimuthal: true,
	toNormal: ([ lat, lon ]) => {
		...
		const rad = Math.sin(D90 - lat)*0.5;
		const x = 0.5 + Math.sin(lon)*rad;
		const y = 0.5 - Math.cos(lon)*rad;
		return [ x, y ];
	},
	toLatLon: ([ x, y ]) => {
		...
		const lonAbs = Math.acos(-dy/len);
		const lon = dx >= 0 ? lonAbs : - lonAbs;
		const lat = D90 - Math.asin(len*2);
		return [ lat, lon ];
	},
});
